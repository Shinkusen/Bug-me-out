...
func iniciar_animacao_pulsar():
	...
	tween.tween_property(label, "scale", Vector2(1.05, 1.05), 0.6)
	tween.tween_property(label, "scale", Vector2(0.95, 0.95), 0.6)
...
#	label.mouse_filter = Control.MOUSE_FILTER_IGNORE
#
## parâmetro removido: cor: Color = Color.WHITE
#func exibir(mensagem: String, tempo_duracao: float = 5.0):
#	label.text = mensagem
#	# label.modulate = cor
#	
